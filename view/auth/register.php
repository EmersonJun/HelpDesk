...
<style>
  .register-page { min-height:100vh; display:flex; align-items:center; justify-content:center;
                   background:linear-gradient(135deg,#0f172a 0%,#1e3a5f 100%); padding:24px; }
  .register-card { background:#fff; border-radius:16px; padding:36px 40px;
  ...
  .step-bar span { flex:1; height:4px; border-radius:2px; background:#e2e8f0; transition:background .3s; }
...
<div class="register-card">
  ...
  <form method="POST" action="<?= APP_URL ?>/?c=auth&a=salvarRegistro" id="registerForm" novalidate>
    ...
    <div class="field-group active" id="etapa1">
      <p class="text-muted small mb-3">
        <span class="badge bg-primary me-1">1 de 3</span> Dados de identidade
      </p>
      ...
      <div class="mb-3">
        ...
        <div class="input-group">
          ...
          <input type="text" name="cpf" id="fCpf" class="form-control"
                 placeholder="000.000.000-00" maxlength="14"
                 value="<?= htmlspecialchars($dados['cpf'] ?? '') ?>"
                 oninput="mascaraCpf(this)">
        </div>
        <div class="form-text small">
          <i class="bi bi-info-circle me-1 text-warning"></i>Usado para recuperação de senha.
        </div>
        <div class="invalid-feedback d-block" id="erroCpf"></div>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="input-group">
          ...
          <input type="date" name="data_nascimento" id="fNasc" class="form-control"
                 value="<?= htmlspecialchars($dados['data_nascimento'] ?? '') ?>">
        </div>
        <div class="invalid-feedback d-block" id="erroNasc"></div>
      </div>

      <button type="button" class="btn btn-primary w-100 py-2 fw-semibold" onclick="irEtapa(2)">
    ...
    <div class="field-group" id="etapa2">
      ...
      <div class="mb-3">
        ...
        <div class="input-group">
          ...
          <input type="password" name="senha" id="fSenha" class="form-control"
                 placeholder="Mínimo 6 caracteres" oninput="verificarSenha()">
        ...
        <div id="strengthLabel" style="font-size:.75rem;margin-top:3px"></div>
      ...
      <div class="mb-4">
        ...
        <div class="input-group">
          ...
          <input type="password" name="confirmacao" id="fConf" class="form-control"
                 placeholder="Repita a senha" oninput="verificarConfirmacao()">
      ...
      <div class="d-flex gap-2">
        <button type="button" class="btn btn-outline-secondary flex-fill" onclick="irEtapa(1)">
          <i class="bi bi-arrow-left me-1"></i> Voltar
        </button>
        <button type="button" class="btn btn-primary flex-fill py-2 fw-semibold" onclick="irEtapa(3)">
          Continuar <i class="bi bi-arrow-right ms-1"></i>
        </button>
      </div>
    </div>
    ...
    <div class="field-group" id="etapa3">
      ...
      <div class="bg-light rounded p-3 mb-3 small">
        ...
        <div><i class="bi bi-person me-1 text-primary"></i><span id="resumoNome" class="fw-semibold"></span></div>
        <div><i class="bi bi-envelope me-1 text-primary"></i><span id="resumoEmail"></span></div>
        <div><i class="bi bi-person-vcard me-1 text-primary"></i><span id="resumoCpf"></span></div>
        <div><i class="bi bi-calendar-date me-1 text-primary"></i><span id="resumoNasc"></span></div>
        <div><i class="bi bi-shield-check me-1 text-success"></i>Perfil:
          <span class="badge bg-info text-dark">Cliente</span></div>
      </div>

      <div class="d-flex gap-2">
        <button type="button" class="btn btn-outline-secondary flex-fill" onclick="irEtapa(2)">
          <i class="bi bi-arrow-left me-1"></i> Voltar
        </button>
        <button type="submit" class="btn btn-success flex-fill py-2 fw-bold" id="btnCriar">
  ...
  <div class="text-center mt-4">
    ...
    <a href="<?= APP_URL ?>/?c=auth&a=login" class="text-primary fw-semibold small ms-1">Fazer login</a>
  </div>
</div>
...
function validarEtapa(n) {
  if (n === 1) {
    ...
    document.getElementById('erroNasc').textContent  = '';
    if (!nome)  { document.getElementById('erroNome').textContent  = 'Informe seu nome.'; ok = false; }
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
                { document.getElementById('erroEmail').textContent = 'Informe um e-mail válido.'; ok = false; }
    if (!cpf || cpf.replace(/\D/g,'').length < 11)
                { document.getElementById('erroCpf').textContent   = 'Informe um CPF válido.'; ok = false; }
    if (!nasc)  { document.getElementById('erroNasc').textContent  = 'Informe sua data de nascimento.'; ok = false; }
    return ok;
  }
  if (n === 2) {
...
function verificarSenha() {
  ...
  document.getElementById('req6').classList.toggle('ok', s.length >= 6);
  document.getElementById('reqUpper').classList.toggle('ok', /[A-Z]/.test(s));
  document.getElementById('reqNum').classList.toggle('ok', /[0-9]/.test(s));
  let score = [s.length>=6,s.length>=10,/[A-Z]/.test(s),/[0-9]/.test(s),/[^a-zA-Z0-9]/.test(s)].filter(Boolean).length;
  const lvs = [{w:'20%',bg:'#ef4444',txt:'Muito fraca'},{w:'40%',bg:'#f97316',txt:'Fraca'},
               {w:'60%',bg:'#f59e0b',txt:'Média'},{w:'80%',bg:'#10b981',txt:'Forte'},{w:'100%',bg:'#059669',txt:'Muito forte'}];
  const lv = lvs[Math.min(score,4)];
  bar.style.width = lv.w; bar.style.background = lv.bg;
  lbl.textContent = s ? lv.txt : ''; lbl.style.color = lv.bg;
}

function verificarConfirmacao() {
  const s = document.getElementById('fSenha').value;
  const c = document.getElementById('fConf').value;
  document.getElementById('erroConf').textContent = (c && s !== c) ? 'As senhas não coincidem.' : '';
}
...
function preencherResumo() {
  ...
  document.getElementById('resumoCpf').textContent   = document.getElementById('fCpf').value;
  const nasc = document.getElementById('fNasc').value;
  if (nasc) {
    const [y,m,d] = nasc.split('-');
    document.getElementById('resumoNasc').textContent = `${d}/${m}/${y}`;
  }
}
